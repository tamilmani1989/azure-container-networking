...
func addStateRulesToMap() map[string]string {
	...
	for _, value := range stateMapkey {
		rulesMap[value] = ebtables.AzurePreRouting
	}

	rulesMap["-s Unicast -o eth0 -j snat --to-src 00:0d:12:3a:5d:32 --snat-arp --snat-target ACCEPT"] = ebtables.AzurePostRouting
	return rulesMap
}
...
func TestAddMissingRule(t *testing.T) {
	...
	for key, value := range currentStateRulesMap {
		if value != ebtables.AzurePostRouting {
